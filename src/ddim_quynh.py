...
def main(args):
    ...
    image_dir = "data/horse"
    #   [B, T, 4, 64, 64]
    # prompts = ["a horse is walking"]
    prompts = [""]
    with torch.cuda.amp.autocast(dtype=torch.float32):
        for filename in sorted(os.listdir(image_dir)):
            t_images = tf(Image.open(f"{image_dir}/{filename}"))
            t_images = (t_images * 2 ) - 1
            gen_images = pipe.vae.encode(t_images.unsqueeze(0).to(dtype=torch.float32, device=pipe.device)).latent_dist.sample() 
            gen_images = gen_images * pipe.vae.config.scaling_factor
            #   Inversion
            ...
            enhance_image_ddim = sample(
                ...
                ["a deer is walking"], 
                start_step=0,
                start_latents=new_start_latent.to(dtype=torch.float32, device=pipe.device),
                guidance_scale=1,
                num_inference_steps=args.inversion_steps,
                num_images_per_prompt=1,
                do_classifier_free_guidance=True,
            ...
            #   Decode image
            decode_latents = torch.cat([gen_images, enhance_image_ddim])
            decode_latents = decode_latents / pipe.vae.config.scaling_factor 
            # images = pipe.decode_latents(decode_latents)                    #   This return a numpy array
            images = (pipe.vae.decode(decode_latents).sample + 1) / 2
            # breakpoint()
            # images = torch.from_numpy(images).permute(0,3,1,2)
            
            #   Save results
            save_image(
                make_grid(images, nrow=len(prompts)), f"{args.save_dir}/{filename}"
            )
