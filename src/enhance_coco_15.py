...
def read_data(file_path):
    ...
    with open(file_path, mode='r') as file:
        ...
        for row in csv_reader:
            image_ids.append(row['file_name'])
            prompts.append(row['text'])
    
    return image_ids, prompts
...
def main(args):
    ...
    def process_image(image_id, input_prompt, model, args):
        path = f"{args.image_folder}/{image_id}"
        gen_image = transform(Image.open(path)).unsqueeze(0).to("cuda")
        enhance_image = model(gen_image, [input_prompt])
        save_image(enhance_image, f"{args.save_dir}/{image_id}")
    image_ids, prompts = read_data("data/coco_30k_15.csv")
    # Use ThreadPoolExecutor for parallel processing
    with ThreadPoolExecutor() as executor:
        futures = [
            executor.submit(process_image, image_id, input_prompt, enhance_model, args)
            for image_id, input_prompt in zip(image_ids, prompts)
        ]
